...
use prbot_database_interface::{DatabaseError, DbService, Result};
use prbot_models::{
...
use crate::row::{
    ...
    PullRequestRuleRow, RepositoryRow, RequiredReviewerRow,
};
...
impl PostgresDb {
    ...
    fn wrap_unknown_pull_request_rule(e: sqlx::Error, name: &str) -> DatabaseError {
        Self::wrap_row_not_found(e, DatabaseError::UnknownPullRequestRule(name.into()))
    }
    ...
    async fn pull_request_rules_get_from_id(&self, id: i32) -> Result<Option<PullRequestRule>> {
        let row = sqlx::query_as::<_, PullRequestRuleRow>(
            r#"
            SELECT *
            FROM pull_request_rule
            WHERE id = $1
        "#,
        )
        .bind(id)
...
impl DbService for PostgresDb {
    ...
    async fn pull_request_rules_all(&self) -> Result<Vec<PullRequestRule>> {
        let rows = sqlx::query_as::<_, PullRequestRuleRow>(
            r#"
                SELECT *
                FROM pull_request_rule
    ...
    async fn pull_request_rules_list(
        ...
        let rows = sqlx::query_as::<_, PullRequestRuleRow>(
            r#"
                SELECT pull_request_rule.*
                FROM pull_request_rule
                INNER JOIN repository ON (repository_id = repository.id)
                WHERE repository.owner = $1
                AND repository.name = $2
                ORDER BY repository_id, pull_request_rule.name
            "#,
        )
        .bind(owner)
        .bind(name)
    ...
    async fn pull_request_rules_create(&self, rule: PullRequestRule) -> Result<PullRequestRule> {
        ...
        .bind(rule.repository_id as i32)
        .bind(rule.name)
        ...
        self.pull_request_rules_get_from_id(new_id)
            .await
            .map(|x| x.unwrap())
    }
    ...
    async fn pull_request_rules_update(&self, rule: PullRequestRule) -> Result<PullRequestRule> {
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| Self::wrap_unknown_pull_request_rule(e, &rule.name))?
        .get(0);
