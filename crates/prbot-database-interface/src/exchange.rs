...
impl Exchanger {
    pub async fn export_to_json<W: Write>(
        ...
        let data = ExchangeData {
            ...
            pull_request_rules: db_service.pull_request_rules_all().await?,
        };

        serde_json::to_writer_pretty(writer, &data).map_err(|e| {
            DatabaseError::ExchangeJsonError {
                message: e.to_string(),
            }
        })?;
    ...
    pub async fn import_from_json<R: Read>(db_service: &dyn DbService, reader: R) -> Result<()> {
        let data: ExchangeData =
            serde_json::from_reader(reader).map_err(|e| DatabaseError::ExchangeJsonError {
                message: e.to_string(),
            })?;
        ...
        for mut rule in data.pull_request_rules {
            let repo_id = repo_id_map.get(&rule.repository_id).unwrap();
            let repo = repo_map.get(repo_id).unwrap();

            println!(
                "> Importing pull request rule '{}' on repository '{}/{}'",
                rule.name, repo.owner, repo.name
            );

            rule.repository_id = *repo_id;
            Self::create_or_update_pull_request_rule(db_service, repo, rule).await?;
        }
    ...
    async fn create_or_update_pull_request_rule(
        db_service: &dyn DbService,
        repository: &Repository,
        rule: PullRequestRule,
    ) -> Result<PullRequestRule> {
        match db_service
            .pull_request_rules_get(&repository.owner, &repository.name, &rule.name)
            .await?
        {
            Some(r) => Ok(r),
            None => db_service.pull_request_rules_create(rule).await,
        }
    }
}
