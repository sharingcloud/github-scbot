...
impl CommandParser {
    ...
    pub fn parse_command_string_from_comment_line<'a>(
        ...
        if comment.starts_with(&config.name) {
            // Plus one for the '@' symbol
            let (_, command) = comment.split_at(config.name.len());
    ...
    fn test_parse_command_string_from_comment_line() {
        ...
        assert_eq!(
            CommandParser::parse_command_string_from_comment_line(
                &config,
                &format!("{} this-is-a-command", config.name)
            ),
        ...
        assert_eq!(
            CommandParser::parse_command_string_from_comment_line(
                &config,
                &format!("{} lock+ Because I choosed to", config.name)
            ),
