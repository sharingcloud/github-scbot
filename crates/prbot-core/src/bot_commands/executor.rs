...
use super::{
    commands::AdminAddMergeRuleCommand, AdminCommand, BotCommand, CommandHandlingStatus,
    UserCommand,
};
use crate::{
    bot_commands::{
        ...
        Command, CommandContext, CommandExecutionResult, CommandResult, ResultAction,
    },
    use_cases::{
        auth::{CheckIsAdmin, CheckWriteRight},
...
#[async_trait]
pub trait CommandExecutorInterface: Interface {
...
/// Command executor.
#[derive(Component)]
#[shaku(interface = CommandExecutorInterface)]
    ...
    async fn process_command_result<'b>(
        ...
        if command_result.should_update_status {
            ...
            update_pull_request_status
                .run(&ctx.as_core_context(), &ctx.pr_handle(), &upstream_pr)
                .await?;
        }
...
impl CommandExecutor {
    ...
    pub async fn execute_command(
        ...
        if self
            .validate_user_rights_on_command(
                &ctx.as_core_context(),
                ctx.comment_author,
                permission,
                &command,
    ...
    pub async fn validate_user_rights_on_command(
        ...
        match command {
            Command::User(cmd) => match cmd {
                ...
                _ => CheckWriteRight.run(ctx, username, user_permission).await,
            },
            Command::Admin(_) => CheckIsAdmin.run(ctx, username).await,
        }
    }
}
