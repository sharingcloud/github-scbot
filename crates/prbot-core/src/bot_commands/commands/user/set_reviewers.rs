...
use crate::{
    ...
    use_cases::reviews::{AddReviewersInterface, FilteredReviewers, RemoveReviewersInterface},
    Result,
};
...
impl AssignReviewersCommand {
    async fn handle<'a>(&self, ctx: &CommandContext<'a>) -> Result<CommandExecutionResult> {
        let add_reviewers: &dyn AddReviewersInterface = ctx.core_module.resolve_ref();
        let reviewers = add_reviewers
            .run(
                &ctx.as_core_context(),
                &ctx.pr_handle(),
                &self.reviewers,
...
impl UnassignReviewersCommand {
    async fn handle<'a>(&self, ctx: &CommandContext<'a>) -> Result<CommandExecutionResult> {
        let remove_reviewers: &dyn RemoveReviewersInterface = ctx.core_module.resolve_ref();
        remove_reviewers
            .run(&ctx.as_core_context(), &ctx.pr_handle(), &self.reviewers)
            .await?;
